...
export default function SignUpMobile({
  ...
  const handleEmailContinue = async () => {
    ...
    try {
      ...
      if (exists) {
        ...
        router.push(`/sign-in?email=${encodeURIComponent(preEmail.trim())}`);
        return;
      }
      setEmailStep(false);
    } catch {
      toast.error("Something went wrong", {
        description: "Unable to check your email right now. Please try again.",
      });
    } finally {
  ...
  const totalSteps = 8;
  const displayStepNumber = emailStep ? 1 : step === 15 ? 2 : stepNumber + 1;
  const progressValue = emailStep
    ? (1 / totalSteps) * 100
    : step === 15
      ? (2 / totalSteps) * 100
            ...
            <div className="flex flex-col gap-5">
              ...
              >
                <CardContent className="">
                  <p className="font-bold text-white text-5">Individual</p>
